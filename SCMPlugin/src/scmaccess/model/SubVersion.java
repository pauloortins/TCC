...
//		}
	...
	public void exportPath(Node node, String localPath, long initialRevision,
		...
		// salvo
		// esses valores pra fazer a diferença com o diffj
				...
				for (Iterator changedPaths = changedPathsSet.iterator(); changedPaths
					...
					if (entryPath.getPath() != null
						...
						if (isMetricInDatabase(metricaFilter)) {
							...
							System.out.println("Pegou métrica do banco !");							
						} else {
							
							System.out.println("Pegou métrica do diffj !");
	...
	private void listDirectories(String path, Node root, int depth) {
		try {
			...
			while (iterator.hasNext()) {
				 ...
				 */
				/*
				 * Mandar imprimir o conteï¿½do do arquivo if ( entry.getKind()
